...
function Dentists() 
{
    ...
    function handleSelectDentist(id) 
    {
        setSelected(id);
        setActiveTab("overview");
        setdentistsSidebarOpen(false);
    }
    ...
    function handleAddDentist() 
    {
        ...
        const newD = 
        {
            ...
            dailyCommission:  0,
            commissionRate:   Number(addForm.commissionRate) / 100,
            patients:         [],
            pendingRequests:  [],
            schedule:         "Mon–Fri 8AM–5PM",
        };
    ...
    return (
        <div className="dentists-root">
            ...
            {dentistsSidebarOpen && (
                <div className="dentists-sidebar-overlay" onClick={() => setdentistsSidebarOpen(false)} />
            )}

            <aside className={`dentists-sidebar${dentistsSidebarOpen ? " dentists-sidebar-open" : ""}`}>

                <div className="dentists-sidebar-header">
                    <h1 className="dentists-sidebar-title">
                        <span>👨‍⚕️</span> Dentists
                    </h1>
                </div>

                {/* ── ADD DENTIST BUTTON BELOW HEADER ── */}
                        ...
                        <li key={d.id} className={`dentist-item ${selected === d.id ? "dentist-item-selected" : ""}`} onClick={() => handleSelectDentist(d.id)}>
                            ...
                            <div className="dentist-right">
                                ...
                                {d.pendingRequests.length > 0 && (
                                    <span className="req-badge">{d.pendingRequests.length}</span>
                                )}
                            </div>
                        </li>
                    ))}
            ...
            <main className="dentists-detail">
                {!selectedDentist ? (
                    ...
                    <div className="detail-content">
                        ...
                        {/* ── HERO: edit + archive icon buttons ── */}
                            ...
                            <div className="detail-hero-right">
                                ...
                                <div className="commission-box">
                                    ...
                                    <span className="commission-value">
                                        ₱{selectedDentist.dailyCommission.toLocaleString()}
                                    </span>
                                    <span className="commission-rate">
                                        {(selectedDentist.commissionRate * 100).toFixed(0)}% rate
                                    </span>
                                </div>
                            </div>
                        </div>

                        {/* ── TABS (Schedule added, no more action bar) ── */}
                            ...
                            <div className="tab-panel">
                                <div className="overview-grid">
                                    <div className="ov-card ov-card-highlight">
                                        ...
                                        <div>
                                            <span className="ov-value">₱{selectedDentist.dailyCommission.toLocaleString()}</span>
                                            <span className="ov-label">Daily Commission</span>
                                        </div>
                                    </div>
                                    <div className="ov-card">
                                        <span className="ov-icon">🧑‍🦷</span>
                                        <div>
                                            <span className="ov-value">{selectedDentist.patients.length}</span>
                                            <span className="ov-label">Handled Patients</span>
                                        </div>
                                    </div>
                                    <div className="ov-card">
                                        <span className="ov-icon">📩</span>
                                        <div>
                                            <span className="ov-value">{selectedDentist.pendingRequests.length}</span>
                                            <span className="ov-label">Patient Requests</span>
                                        </div>
                                    </div>
                                    <div className="ov-card">
                                        <span className="ov-icon">📊</span>
                                        <div>
                                            <span className="ov-value">{(selectedDentist.commissionRate * 100).toFixed(0)}%</span>
                                            <span className="ov-label">Commission Rate</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="section-card">
                                    ...
                                    <div className="commission-table">
                                        ...
                                        {selectedDentist.patients.length === 0 && (
                                            <p className="no-data">No commission data yet.</p>
                                        )}
                                        {selectedDentist.patients.map((p) => (
                                            <div key={p.id} className="ct-row">
                                                <span>{p.lastVisit}</span>
                                                <span>{p.name}</span>
                                                <span className="ct-procedure">{p.procedure}</span>
                                                <span className="ct-amount">₱{p.amount.toLocaleString()}</span>
                                            </div>
                                        ))}
                                        {selectedDentist.patients.length > 0 && (
                                            <div className="ct-total">
                                                ...
                                                <span className="ct-amount-total">
                                                    ₱{selectedDentist.dailyCommission.toLocaleString()}
                                                </span>
                                            </div>
                                        )}
                                                ...
                                                <li key={req.id} className="request-item">
                                                    ...
                                                    {!action ? (
                                                        <div className="req-actions">
                                                            <button className="btn-accept" onClick={() => handleRequestAction(selectedDentist.id, req.id, "accepted")}>
                                                                Accept
                                                            </button>
                                                            <button className="btn-decline" onClick={() => handleRequestAction(selectedDentist.id, req.id, "declined")}>
                                                                Decline
                                                            </button>
                                                        </div>
                                                    ) : (
                        ...
                        {activeTab === "forms" && (
                            <div className="tab-panel">
                                <p className="forms-intro">
                                    Prepare and e-sign clinical forms for your patients. Choose a template below.
                                </p>
            ...
            {/* ── FORM MODAL ── */}
            {showFormModal && (
                <div className="modal-overlay" onClick={() => setShowFormModal(null)}>
                    <div className="modal modal-form-doc" onClick={(e) => e.stopPropagation()}>

                        <div className="modal-header">
                            <h3>{showFormModal.icon} {showFormModal.label}</h3>
                            <button className="modal-close" onClick={() => setShowFormModal(null)}>✕</button>
                        </div>
                                    ...
                                    <div className="doc-content">
                                        ...
                                        <div className="doc-checkboxes">
                                            {["Periapical", "Bite-wing", "Panoramic (OPG)", "Occlusal", "Cephalometric"].map((x) => (
                                                <label key={x} className="doc-checkbox-item">
                                ...
                                <div className="esign-section">
                                    {!signed ? (
                                        <>
                                            ...
                                            {!esignMode ? (
                                                <button className="btn-esign" onClick={() => setEsignMode(true)}>
                                                    ✍️ Add E-Signature
                                                </button>
                                            ) : (
                                                <form className="esign-form" onSubmit={handleESign}>
                                                    ...
                                                    <div className="esign-btns">
                                                        <button type="button" className="btn-esign-cancel" onClick={() => setEsignMode(false)}>
                                                            Cancel
                                                        </button>
                                                        <button type="submit" className="btn-esign-confirm">
                                                            Sign Document
                                                        </button>
                                                    </div>
                                                </form>
            ...
            {showAddModal && (
                <div className="modal-overlay" onClick={() => setShowAddModal(false)}>
                    <div className="modal dact-edit-modal" onClick={(e) => e.stopPropagation()}>
                        <div className="modal-header">
                            <h3>➕ Add Dentist</h3>
                        ...
                        <div className="dact-modal-body">
                            <div className="dact-form-grid">
                                <div className="dact-form-group">
                                    ...
                                    <input value={addForm.name} onChange={(e) => setAddForm((f) => ({ ...f, name: e.target.value }))} placeholder="Dr. Full Name" />
                                </div>
                                <div className="dact-form-group">
                                    <label>Specialty *</label>
                                    <input value={addForm.specialty} onChange={(e) => setAddForm((f) => ({ ...f, specialty: e.target.value }))} placeholder="e.g. Orthodontics" />
                                </div>
                                <div className="dact-form-group">
                                    <label>Contact</label>
                                    <input value={addForm.contact} onChange={(e) => setAddForm((f) => ({ ...f, contact: e.target.value }))} placeholder="+63 9XX XXX XXXX" />
                                </div>
                                <div className="dact-form-group">
                                    <label>Email</label>
                                    <input type="email" value={addForm.email} onChange={(e) => setAddForm((f) => ({ ...f, email: e.target.value }))} placeholder="email@clinic.com" />
                                </div>
                                <div className="dact-form-group">
                                    <label>Commission Rate (%)</label>
                                    <input type="number" value={addForm.commissionRate} onChange={(e) => setAddForm((f) => ({ ...f, commissionRate: e.target.value }))} placeholder="30" />
                                </div>
                                <div className="dact-form-group">
                                    <label>Status</label>
                    ...
                    <div className="modal dact-edit-modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="dact-modal-body">
                            <div className="dact-form-grid">
                                <div className="dact-form-group">
                                    <label>Full Name</label>
                                    <input defaultValue={selectedDentist?.name} placeholder="Dr. Full Name" />
                                </div>
                                <div className="dact-form-group">
                                    <label>Specialty</label>
                                    <input defaultValue={selectedDentist?.specialty} placeholder="Specialty" />
                                </div>
                                <div className="dact-form-group">
                                    <label>Contact</label>
                                    <input defaultValue={selectedDentist?.contact} placeholder="+63 9XX XXX XXXX" />
                                </div>
                                <div className="dact-form-group">
                                    <label>Email</label>
                                    <input defaultValue={selectedDentist?.email} placeholder="email@clinic.com" />
                                </div>
                                <div className="dact-form-group">
                                    <label>Commission Rate (%)</label>
                                    <input type="number" defaultValue={(selectedDentist?.commissionRate * 100).toFixed(0)} placeholder="30" />
                                </div>
                                <div className="dact-form-group">
                                    <label>Status</label>
                                    <select defaultValue={selectedDentist?.status}>
            ...
            {showArchiveConfirm && (
                <div className="modal-overlay" onClick={() => setShowArchiveConfirm(false)}>
                    <div className="modal dact-confirm-modal" onClick={(e) => e.stopPropagation()}>
                        <div className="modal-header">
